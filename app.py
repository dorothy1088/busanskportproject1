...
st.set_page_config(page_title="북항/신항 환적 효율 대시보드", layout="wide")
...
kpi_empty_bokhang=empty_all.loc[('북항',True),'공컨비율']
kpi_empty_gap=(kpi_empty_sinhang-kpi_empty_bokhang)
###
...
with col3:
    st.metric(
        '환적 공건비율 격차',
        value=f'{kpi_empty_gap:.2f}%p', #변화향 때문에 -1했기에 퍼센트 포인트로
        border=True,
    )
...
with col4:
    st.metric(
        '물동량-TEU Factor 상관계수',
        value=f'신항{kpi_corr_sinhang:.2f}, 북항{kpi_corr_bukhang:.2f}', border=True,
    )
###
